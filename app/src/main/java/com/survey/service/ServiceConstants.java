package com.survey.service;

/**
 * Created by Purnendu Mishra on 4/15/2017.
 */
public class ServiceConstants {
    public static final String SIGN_IN_REQUEST ="register.php" ;
    public static final String family_bussiness="family_bussiness";
    public static final String family_cast="family_cast";
    public static final String time="time";
    public static final String education="education";
    public static final String house_no="house_no";
    public static final String member_count="member_count";
    public static final String member_detail="member_detail";
    public static final String mobile_no="mobile_no";
    public static final String family_no="family_no";
    public static final String owner_name="owner_name";
    public static final String family_time="family_time";
    public static final String device_id="device_id";

    public interface WebConstants {

        String IMAGE_PATH="image_path";

        String DEVICE_TYPE="DeviceType";
        String DEVICE_ID="DeviceId";
        String USER_ROLE="UserRole";
        String FRIST_NAME="first_name";
        String LAST_NAME="last_name";
        String DOB="dob";
        String NICK_NAME="nick_name";
        String EMAIL="email";
        String PASSWORD="password";
        String CONFIRM_PASSWORD="confirm_password";
        String PHONE_NUMBER="phone_number";
        String HOME_ADDRESS="home_address";
        String APARTMENT="apt";
        String CITY="city";
        String STATE="state";
        String IS_SAME_ADDRESS="is_same_address";
        String INTERVIEW_CITY="interview_city";
        String INTERVIEW_STATE="interview_state";
        String INTERVIEW_ADDRESS="interview_address";
        String INTERVIEW_APT="interview_apt";
        String REFERRED_BY="referred_by";
        String IMAGE_ID="image_id";

        String ACCESS_TOKEN="access_token";
        String CLIENT="client";
        String CANDIDATE="candidate";


        String BASE_URL_CONSTANT= true? "https://propitious-leather.000webhostapp.com/":"https://propitious-leather.000webhostapp.com/";


    }






}
